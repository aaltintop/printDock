...
export SHOPIFY_BILLING_MODE="${SHOPIFY_BILLING_MODE:-managed}"
...
# Billing: managed = Shopify-hosted plans (default). Set to api only for local Billing API dev.
export SHOPIFY_BILLING_MODE="$SHOPIFY_BILLING_MODE"
EOF
...
Cloud Run variables exported in current shell:
  ...
  SHOPIFY_BILLING_MODE=$SHOPIFY_BILLING_MODE
